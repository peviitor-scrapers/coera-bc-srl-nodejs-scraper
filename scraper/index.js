...
async function fetchJobsPage() {
  ...
  if (!res.ok) {
    throw new Error(`COERA careers HTTP ${res.status}`);
  }
...
const ROMANIAN_CITIES = [
  "București", "Bucuresti", "Cluj-Napoca", "Brașov", "Brasov",
  "Timișoara", "Timisoara", "Iași", "Iasi", "Sibiu", "Oradea", "Constanța", "Constanta"
];

const CITY_ALIASES = { "cluj": "Cluj-Napoca" };
...
function extractCities(title) {
  ...
  for (const [alias, canonical] of Object.entries(CITY_ALIASES)) {
    if (new RegExp(`\\b${alias}\\b`, "i").test(locationPart)) {
      found.add(canonical);
    }
  }
  return [...found];
...
function parsePageJobs(html) {
  ...
  $("a.careerButton").each((_, el) => {
    ...
    const location = cities.length > 0 ? cities : [scraperConfig.defaultLocation];
...
async function scrapeAllListings(testOnlyOnePage = false) {
  ...
  console.log(`Total jobs on COERA: ${total}`);
...
export { parsePageJobs, extractCities, mapToJobModel, transformJobsForSOLR };
