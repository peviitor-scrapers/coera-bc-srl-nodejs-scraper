...
async function fetchCoeraCareers() {
...
describe('E2E: Full Scraping Pipeline', () => {
  ...
  describe('COERA — Real Data Fetch', () => {
    let html;

    beforeAll(async () => {
      const res = await fetchCoeraCareers();
      html = await res.text();
    }, 15000);

    it('should respond with valid HTML containing careerButton anchors', () => {
    ...
    it('should contain job links', () => {
      expect(html).toMatch(/href="\/careers\/[^"]*"/);
    });
  });
  ...
  describe('Parse + Transform Pipeline', () => {
    ...
    it('should parse real COERA careers HTML into standardized format', () => {
      ...
      expect(parsed.url).toMatch(/^https:\/\/www\.co-era\.com\//);
    ...
    it('should map parsed jobs to job model', () => {
      ...
      expect(model.url).toMatch(/^https:\/\/www\.co-era\.com\//);
    });
  ...
  describe('Company Validation Path', () => {
    ...
    itIfAnaf('should find COERA in ANAF and validate active status', async () => {
      const results = await anaf.searchCompany(TEST_BRAND);

      const coera = results.find(c =>
        c.cui.toString() === TEST_CIF &&
        c.statusLabel === 'Funcțiune'
      );
      expect(coera).toBeDefined();
      expect(coera.cui.toString()).toBe(TEST_CIF);
    }, 30000);
    ...
    itIfApi('should run full validation and report active status with job count', async () => {
      ...
      if (result.existingJobsCount === 0) {
        console.log('⚠️ No COERA jobs in API — skipping job count assertion');
        return;
      }
  ...
  describe('API Data Verification', () => {
    ...
    itIfApi('should have COERA jobs in API with correct company name', async () => {
      const result = await api.querySOLR(TEST_CIF);

      if (result.numFound === 0) {
        console.log('⚠️ No COERA jobs in API — skipping API data verification');
        return;
      }
    ...
    itIfApi('should have COERA company core entry with required fields', async () => {
