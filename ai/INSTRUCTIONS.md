...
Target: https://www.co-era.com/careers
...
1. **Start with brand** - We know the brand ("COERA")
...
8. **Scrape new jobs** - Extract jobs from Qualitest Careers API (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Qualitest Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Qualitest Careers API (Romania only)
...
- **QualityAI Careers**: `GET https://www.co-era.com/careers/jobs/search?q=Bucharest&searchby=location` — SSR HTML with job links
